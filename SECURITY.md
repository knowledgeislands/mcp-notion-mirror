...
6. **Nothing reachable from a tool writes to stdout.** The server speaks JSON-RPC over stdout; the orchestrator API layer returns structured data and never logs (only the CLI, which is not a tool, prints). A stray stdout write would corrupt the protocol stream — tests assert the API layer is silent.
